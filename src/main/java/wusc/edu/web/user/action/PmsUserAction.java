package wusc.edu.web.user.action;
...
import wucs.edu.facade.user.enums.UserStatusEnum;
import wucs.edu.facade.user.enums.UserTypeEnum;
import wusc.edu.demo.user.biz.PmsUserService;
import wusc.edu.web.user.base.BaseAction;
...
public class PmsUserAction extends BaseAction {
    ...
    public String changeUserStatus() {
        try {
            Long userId = getLong("id");
            PmsUser user = pmsUserService.getById(userId);
            if (user == null) {
    ...
    public String resetUserPwdUI() {
        PmsUser user = pmsUserService.getById(getLong("id"));
        if (user == null) {
    ...
    public String resetUserPwd() {
        try {
            ...
            pmsUserService.updateUserPwd(userId, DigestUtils.sha1Hex(newPwd), false);
